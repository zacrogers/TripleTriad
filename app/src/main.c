    ...
    for(uint8_t move = 0; move < N_MOVES; ++move)
    {
        for(uint8_t turn = 0; turn < 2; ++ turn)
        {
            if(TT_PLAYER_A == turn)
            {
                ...
                bool placed = tt_place_card(turn, hand_pos, board_x, board_y);
                if(placed)
                {
...
int main(void)
{
    run_moves(player_start_hand, enemy_start_hand, player_moves, enemy_moves);
    return 0;
}
